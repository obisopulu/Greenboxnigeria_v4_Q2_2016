...
<button title='Play' style='font-size:0px; background:url(images/mobplay.png) center no-repeat; border:none; background-size:cover; width:100px; height:100px; cursor:pointer; background-color:#222; font-size:0px' 
		onClick="play('<?php echo substr_replace($songTitle,'',15); ?>','<?php echo "songs/".$songURL; ?>','<?php echo "songsimg/".$songArt; ?>','<?php echo substr_replace($songArtist,'',15); ?>','<?php echo substr_replace($songArtistFt,'',15); ?>','<?php echo substr_replace($songProducer,'',15); ?>','<?php echo $songAlbum; ?>','<?php echo $songID; ?>')"></button>
 </td>
 
<td width='250px' style='background-color:#222;' align='center'>Copies<span style='font-size:1.7em; color:#FFF'><br/><?php echo $copies ?></span><br/><button id='demo<?php echo $songID?>' onclick="copyToClipboard(document.getElementById('demo<?php echo $songID ?>').innerHTML, '<?php echo $songID ?>')"style='font-size:0px; background:url(images/mobcopy.png) center no-repeat; border:none; background-size:cover; width:100px; height:100px; cursor:pointer; background-color:#222; font-size:0px' title='copy file url'>http://www.gbngr.com/library.php?1=filepreview.php?songURL=<?php echo $songURL ?></button>
</td>
        
<td width='250px' style='background-color:#222;' align='center'>Downloads<span style='font-size:1.7em; color:#FFF'><br/><?php echo $songDownload ?></span><br/><form action='stats.php' method='post' target="_blank">
